...
class TimelineSer(serializers.ModelSerializer): 
    ...
    class Meta:
        model = Profile
        fields = ('name', 'last_name', 'picture', 'gender', 'user', 'posts')
        extra_kwargs = {"picture": {"required": False, "allow_null": True}}


    def get_posts(self, value):
        qs = Post.objects.filter(user=value.user)
...
class FriendsPostSerializer(serializers.ModelSerializer): 
    ...
    class Meta:
        model = Post
        fields = ('body', 'image', 'id','userprof','comment', 'comment_count', 'liked', 'like_count')
        extra_kwargs = {"image": {"required": False, "allow_null": True}}
    ...
    def get_comment(self, value):
        serializer = CommentSerializer(instance=reversed(value.comments.all().order_by('-id')[:2]), many=True)
        return serializer.data
...
class SearchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Profile
        fields = ('name', 'last_name', 'picture', 'user', 'isfriend', 'sentreq', 'recreq')
